...
- Terminal scrollbars are styled with OverlayScrollbars without adding a proxy scrolling container. The OverlayScrollbars target and scrollbar slot are the pane's `.terminal-mount`, while the viewport and content are xterm's own `.xterm-viewport` and `.xterm-scroll-area`. xterm still owns the scroll offset, wheel handling, selection behavior, and fit measurements.
- Reattach or update the OverlayScrollbars instance whenever an existing xterm element is remounted into a new pane container. Pane moves, tab switches, and layout rekeys can reuse the xterm instance while changing the DOM parent.
- Do not initialize OverlayScrollbars with the xterm root as the target. In real Tauri/WKWebView this can rewrite xterm's DOM ownership enough to produce a blank terminal or broken input. Targeting only `.xterm-viewport` preserves display and input, but can make the generated scrollbar slot unreliable or invisible. Use `.terminal-mount` as the slot and xterm's viewport/content as the actual scroll elements.
- OverlayScrollbars' native overflow detection is not sufficient for xterm in WKWebView because xterm's scrollback is a terminal buffer state, not just ordinary DOM overflow. After every xterm scroll/write/fit update, mirror `baseY`, `viewportY`, and `rows` into the generated vertical scrollbar classes and CSS variables so the thumb stays visible and positioned without introducing a proxy scroll container.
- OverlayScrollbars-generated scrollbar pointer interactions must also route back through xterm. Disable OverlayScrollbars `dragScroll` for terminal panes and map track/handle pointer positions to `scrollToLine`; otherwise dragging or clicking the generated thumb can mutate DOM scroll state without updating xterm's buffer viewport, leaving the thumb hidden or stuck at the top.
...
- In real Tauri/WebKit validation, verify the scrollbar visually after long output. If a bright native/default bar appears, style both the OverlayScrollbars theme variables and the generated `.os-scrollbar-handle`; relying only on inherited terminal color variables can miss scrollbar elements created outside the xterm subtree.
