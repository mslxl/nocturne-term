...
export const settingsSchema: SettingDefinition[] = [
  ...
  {
    key: "files.tree_sticky_enabled",
    category: "files",
    label: "filesTreeStickyEnabled",
    path: ["files", "tree_sticky_enabled"],
    kind: "boolean",
    defaultValue: true,
    help: "filesTreeStickyEnabledHelp",
    get: (root) => booleanValue(valueAt(root, ["files", "tree_sticky_enabled"])) ?? true,
    toConfigValue: (value) => configBoolean(Boolean(value)),
  },
  {
    key: "files.tree_sticky_max_levels",
    category: "files",
    label: "filesTreeStickyMaxLevels",
    path: ["files", "tree_sticky_max_levels"],
    kind: "select",
    defaultValue: "3",
    options: [
      { value: "1", label: "oneLevel" },
      { value: "2", label: "twoLevels" },
      { value: "3", label: "threeLevels" },
      { value: "4", label: "fourLevels" },
      { value: "5", label: "fiveLevels" },
    ],
    help: "filesTreeStickyMaxLevelsHelp",
    get: (root) => String(integerValue(valueAt(root, ["files", "tree_sticky_max_levels"])) ?? 3),
    toConfigValue: (value) => configInteger(Number(value)),
  },
  {
