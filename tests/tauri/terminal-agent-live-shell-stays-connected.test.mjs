 ...
 *
 * Feature:
 * Verifies a local Terminal Agent session for a normal interactive shell stays
 * connected and accepts input instead of opening as a history/disconnected view.
 *
 * Operation:
 * Seeds an isolated local host with Terminal Agent mode enabled and a shell
 * command that remains alive, launches the real Tauri app, writes an echo
 * command through the Tauri terminal command, and inspects the Terminal ToolTab.
 *
 * Expected:
 * The Terminal ToolTab is agent-backed, remains running, has no initial
 * Disconnected banner, records no registry exit, and echoes the written input.
 */
...
test("terminal agent live shell stays connected and writable", { timeout: 420_000 }, async () => {
  const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
  const appPath = await resolveTauriTestApplication(repoRoot);
  const isolatedAppConfig = await createIsolatedAppConfigEnv("terminal-agent-live-shell-stays-connected");
  ...
  try {
    ...
    await waitUntil(
      async () => {
        const state = await terminalState();
        return state.statuses.length === 1 && state.statuses[0] === "running";
      },
      async () => `Live Terminal Agent shell did not stay running\n${JSON.stringify(await terminalState(), null, 2)}\n${JSON.stringify(await registryFiles(), null, 2)}\n${await pageSummary()}\n${driverOutput}`,
    );

    const mounted = await terminalState();
    if (mounted.readOnly.some((value) => value === "true")) {
      throw new Error(`Live Terminal Agent shell must not be read-only\n${JSON.stringify(mounted, null, 2)}`);
    }
    if (/\[Disconnected(?::|\])/.test(mounted.terminalText) || /\[Disconnected(?::|\])/.test(mounted.bodyText)) {
      throw new Error(`Live Terminal Agent shell rendered a disconnected banner\n${JSON.stringify(mounted, null, 2)}\n${await pageSummary()}`);
    }
    const registries = await registryFiles();
    if (registries.length !== 1 || registries[0].content.includes("[exit]")) {
      throw new Error(`Live Terminal Agent registry should exist without exit metadata\n${JSON.stringify(registries, null, 2)}`);
    }

    const marker = "NOCTURNE_AGENT_LIVE";
    await writeTerminal(`${echoCommand(marker)}\r`);
    await waitUntil(
      async () => {
        const state = await terminalState();
        return state.statuses[0] === "running" && state.terminalText.includes(marker);
      },
      async () => `Live Terminal Agent shell did not echo written input\n${JSON.stringify(await terminalState(), null, 2)}\n${JSON.stringify(await registryFiles(), null, 2)}\n${await pageSummary()}\n${driverOutput}`,
    );
  } finally {
    if (sessionId) {
      await deleteMountedAgentSession().catch(() => undefined);
      await waitUntil(
        async () => (await registryFiles()).length === 0,
        "live terminal agent session did not exit during test cleanup",
        10_000,
      ).catch(() => undefined);
    }
    if (sessionId) {
  ...
  async function configureFixtureHost() {
    ...
    await writeFile(
      ...
      [
        "version = 1",
        `id = "${fixtureHostId}"`,
        `name = "Live Shell Host"`,
        `protocol = "local"`,
        "",
        "[local]",
        `command = ${JSON.stringify(liveShellCommand())}`,
        `args = ${tomlStringArray(liveShellArgs())}`,
        "env = {}",
        "",
        "[terminal]",
        `agent_mode = "enabled"`,
  ...
  function liveShellCommand() {
    return process.platform === "win32" ? "cmd.exe" : "/bin/sh";
  }

  function liveShellArgs() {
    return process.platform === "win32" ? ["/d", "/q", "/k", "prompt $G"] : ["-i"];
  }

  function echoCommand(marker) {
    return process.platform === "win32" ? `echo ${marker}` : `printf '${marker}\\n'`;
  }
  ...
  async function writeTerminal(data) {
    const input = await execute(`
      ...
      const sessionId = surface?.getAttribute('data-session-id') ?? '';
      if (!sessionId) return { error: 'missing terminal session id' };
      return { session_id: sessionId, data: arguments[0] };
    `, [data]);
    if (input.error) {
      throw new Error(`Cannot build terminal input: ${JSON.stringify(input, null, 2)}`);
    }
    await invokeOk("write_terminal", { input });
  }

  async function deleteMountedAgentSession() {
    const result = await execute(`
      const surface = document.querySelector('[data-testid="terminal-surface"]');
      const agentSessionId = surface?.getAttribute('data-agent-session-id') ?? '';
      ...
      if (!workspaceId || !toolTabId || !agentSessionId) {
        return { error: 'missing workspace, ToolTab, or agent session id', snapshot, toolTabId, agentSessionId };
      }
      return {
        input: {
          workspace_id: workspaceId,
          tool_tab_id: toolTabId,
          detached_session_id: agentSessionId,
        },
      };
    `);
    if (result.error) {
      throw new Error(`Cannot build terminal delete input: ${JSON.stringify(result, null, 2)}`);
    }
    await withTimeout(
      invokeOk("delete_detached_terminal_session", { input: result.input }),
      15_000,
      async () => `delete_detached_terminal_session did not return during cleanup\n${await pageSummary()}\n${driverOutput}`,
    );
  }
  ...
  async function terminalState() {
    return await execute(`
      ...
      return {
        ...
        sessionIds: surfaces.map((surface) => surface.getAttribute('data-session-id') ?? '').filter(Boolean),
