 ...
 * column-count changes use a resize motion without horizontal track travel,
 * resize keeps the current pane identity stable so existing column scroll
 * viewports are not recreated, and animation cleanup collapses back to one
 * current pane after the motion.
 */
...
describe("Files Columns view motion source", () => {
  it("slides between previous and next three-column panes", async () => {
    ...
    assert.match(source, /\.columns-content\.motion-resize\s+\.file-column\s*\{[^}]*flex-basis/s);
