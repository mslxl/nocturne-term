...
fn normalize_snapshot_group_roles(snapshot: &mut WorkspaceLayoutSnapshot) {
    for workspace in &mut snapshot.workspaces {
        normalize_layout_group_roles(&mut workspace.layout, DockRoleBounds::root());
    }
    for window in &mut snapshot.floating_windows {
...
fn normalize_layout_group_roles(layout: &mut WorkspaceDockLayout, bounds: DockRoleBounds) {
    match layout {
        WorkspaceDockLayout::Group { role, slots, .. } => {
            *role = if slots.is_empty() {
                WorkspaceDockGroupRole::Content
            } else {
                dock_role_for_bounds(bounds)
            };
        }
        WorkspaceDockLayout::Split {
            direction,
            ...
            for (index, child) in children.iter_mut().enumerate() {
                let child_bounds = match direction {
                    WorkspaceDockDirection::Row => DockRoleBounds {
                        left: bounds.left && index == 0,
                        right: bounds.right && index == last,
                        top: bounds.top,
                        bottom: bounds.bottom,
                    },
                    WorkspaceDockDirection::Column => DockRoleBounds {
                        left: bounds.left,
                        right: bounds.right,
                ...
                normalize_layout_group_roles(child, child_bounds);
            }
        }
    }
}

fn dock_role_for_bounds(bounds: DockRoleBounds) -> WorkspaceDockGroupRole {
