...
pub(crate) fn build_menu<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<Menu<R>> {
    ...
    let profile_new = MenuItem::with_id(app, MENU_PROFILE_NEW, "New...", true, None::<&str>)?;
    let profile_edit = MenuItem::with_id(app, MENU_PROFILE_EDIT, "Edit...", true, None::<&str>)?;
    let profile_delete =
        MenuItem::with_id(app, MENU_PROFILE_DELETE, "Delete...", true, None::<&str>)?;
    ...
    let profile_menu = Submenu::with_items(app, "Profile", true, &profile_children)?;
    let close = PredefinedMenuItem::close_window(app, None)?;
    let file = Submenu::with_items(app, "File", true, &[&settings, &profile_menu, &close])?;
...
impl DialogKind {
    ...
    fn title(&self) -> &'static str {
        match self {
            Self::ProfileNew => "New Profile",
            Self::ProfileDelete => "Delete Profile",
        }
    }
